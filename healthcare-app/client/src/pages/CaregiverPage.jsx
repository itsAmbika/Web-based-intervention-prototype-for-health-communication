...
import { getCaregiverQuestions, registerCaregiver } from '../services/api'
...
export default function CaregiverPage() {
  ...
  const handleFormReady = async (formPayload) => {
    ...
    setQuestionError('')
    try {
      const res = await getCaregiverQuestions()
  ...
  const handleQuestionsComplete = async ({ selectedQuestions, customQuestion }) => {
    ...
    try {
      ...
      const response = await registerCaregiver(payload)
      setCaregiverId(response.data.caregiverId)
      setStep('done')
    } catch (err) {
      setSubmitError(
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          {step === 'form' && (
            <>
              <h1 className="text-2xl font-bold text-gray-800 mb-6">Caregiver Registration</h1>
              <CaregiverForm onReady={handleFormReady} />
            </>
          )}
          ...
          {step === 'done' && (
            <div className="text-center py-12">
              ...
              <div className="inline-block bg-teal-50 border border-teal-200 rounded-xl px-8 py-4">
                <p className="text-sm text-gray-500 mb-1">Your Caregiver ID</p>
                <p className="text-3xl font-bold text-teal-700 tracking-wide">{caregiverId}</p>
              </div>
              <div className="mt-8">
